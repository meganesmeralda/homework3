...
load("../submission1/results/hwk3_workspace.Rdata")
```
```{r}
#| include: false
...
My answers in the following file. Check out my repository [here](https://github.com/meganesmeralda/homework3). I have suddenly come across many errors all over my code that I am attempting to fix, so sorry if this information looks similar to, or even less than, submission 1.
...
filtered_data_2018 <- final.data %>%
  ...
  summarize(avg_tax = mean(tax_real, na.rm = TRUE),
            avg_price = mean(price_real, na.rm = TRUE), .groups = "drop")
filtered_data_2018_long <- filtered_data_2018 %>%
  pivot_longer(cols = c(avg_tax, avg_price), names_to = "Type", values_to = "Value")

q2 = ggplot(filtered_data_2018_long, aes(x = Year, y = Value, color = Type)) +
  geom_line(size = 1.2) +
